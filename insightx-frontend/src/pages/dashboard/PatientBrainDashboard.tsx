...
const PatientBrainDashboard: React.FC = () => {
  ...
  const exportPdf = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        pdf.save(`InsightX_Report_${scan?.patientId ?? patientId}.pdf`);
      } catch (err) {
  ...
  const patientIdDisplay = scan?.patientId ?? patientId ?? "N/A";
  const scanIdDisplay = activeScan?.id ?? scan?.scanId ?? "N/A";
  const oxygenation =
    scan?.oxygenation !== undefined && scan?.oxygenation !== null
      ? `${scan.oxygenation}%`
      : "N/A";
  const performance =
    scan?.score !== undefined && scan?.score !== null ? scan.score : "N/A";
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between border-b pb-4 border-slate-200 mb-8">
            <div>
              ...
              <p className="text-[11px] text-slate-500 font-mono uppercase">
                ID: {patientIdDisplay} | Scan: {scanIdDisplay}
              </p>
            </div>
            {!isExporting && (
                ...
                <div className="min-h-[400px] flex items-center justify-center bg-slate-900 rounded-2xl overflow-hidden relative shadow-inner">
                  ...
                  {hasVisualization ? (
                    viewMode === "heatmap" ? (
                      ...
                      <MRIViewer
                        volumeUrl={toApiUrl(
                          String(aiResult?.reconstruction_file || "")
                        )}
                      />
                    )
                  ) : (
                ...
                >
                  ...
                  <div className="space-y-2 text-xs text-slate-700">
                    <div>
                      ...
                      <span className="font-bold">
                        {aiResult.series_uid ?? "N/A"}
                      </span>
                    </div>
                    <div>
                      <span className="text-slate-400">Series Detected:</span>{" "}
                      <span className="font-bold">
                        {aiResult.series_detected ?? "N/A"}
                      </span>
                    </div>
                    <div>
